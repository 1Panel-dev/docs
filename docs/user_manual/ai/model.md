...
!!! note ""
    **模型账号** 保存的是"怎么连接某个 AI 模型服务"的信息（服务地址、密钥等），相当于为模型服务存了一份"登录凭证"。之后智能体、AI 搜索等功能要使用模型时，直接从这里选择即可，无需反复填写。

    点击 **创建** 后，按页面提示配置以下信息：

    - **名称**：模型账号在 1Panel 中的显示名称。
    - **模型供应商**：选择预置供应商，或选择自定义、Ollama、vLLM 等类型。
    - **API 类型**：按供应商支持情况选择兼容协议。
    - **Base URL**：模型服务 API 地址。部分预置供应商使用默认地址，自定义、Ollama 和 vLLM 可按实际服务填写。
    - **API Key**：模型服务的访问凭证。是否需要填写取决于供应商。
    - **备注**：记录账号用途等说明信息。
...
!!! note ""
    使用 Ollama 管理前，需要先从 **应用商店** 安装 Ollama。安装完成后，页面会显示应用运行状态，并提供启动、停止、重启和连接信息等操作。若未检测到 Ollama，页面会提示前往应用商店安装。
...
![未检测到 Ollama 提示](../../img/ai/model_local.png)
...
![模型下载器](../../img/ai/model_downloader.png)
