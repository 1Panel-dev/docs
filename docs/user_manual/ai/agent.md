...
# 2. 1Panel Installation and Deployment
To install and deploy 1Panel, first log in to the Linux client or server, then follow the steps below for quick deployment:

## 2.1. Step 1: Obtain root Privileges
After logging in to the server, enter the following command to switch to root privileges first:
```
sudo su -
```

## 2.2. Step 2: Execute the Online Installation Command
Enter the following online installation script command to start the installation:
```
bash -c "$(curl -sSL https://resource.fit2cloud.com/1panel/package/v2/quick_start.sh)"
```

## 2.3. Step 3: Specify the 1Panel Installation Directory
The installation script will first prompt to set a specified installation directory. Press Enter to select the default directory if there are no special requirements:

Set the 1Panel installation directory (default: /opt):

## 2.4. Step 4: Complete Docker Installation
After specifying the directory, the installation script will automatically detect whether Docker is installed on the current server. If not, it will ask whether to install it. Enter "y" here to confirm the installation:

Docker not detected, install it? [y/n]: y
